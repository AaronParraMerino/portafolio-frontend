    ...
    <div className="prf-modal-overlay" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className="prf-modal" style={{ maxWidth: 480 }}>
        ...
        <div className="prf-modal-body" style={{ padding: '20px 22px' }}>
          ...
          {/* Botón secundario para elegir archivo si ya hay preview */}
          {preview && (
            <button
              className="prf-btn-outline"
        ...
        <div className="prf-modal-foot">
          <button className="prf-btn-cancel" onClick={onClose} disabled={cargando}>
            Cancelar
          </button>
          <button
          ...
          >
            {cargando ? (
              ...
              <><svg viewBox="0 0 12 12"><path d="M1 6l3.5 3.5L11 2"/></svg> Guardar {label}</>
            )}
          </button>
        </div>
      </div>
      ...
      <div className="prf-modal" style={{ maxWidth: 400 }}>
        ...
        <div className="prf-modal-body" style={{ padding: '18px 22px' }}>
          <p style={{ fontSize: 13, color: 'var(--gris-oscuro)', lineHeight: 1.6, margin: 0 }}>
            ...
            {tipo === 'banner'
              ? ' El banner volverá al gradiente por defecto.'
              : ' Tu foto de perfil se reemplazará por tus iniciales.'
            }
          </p>
        </div>
        ...
        <div className="prf-modal-foot">
          ...
          <button
            className="prf-btn-save"
            style={{ background: 'var(--rojo-soft)' }}
            onClick={onConfirm}
            disabled={cargando}
          >
            {cargando ? (
              ...
              <>
                <svg viewBox="0 0 12 12" fill="none" stroke="currentColor" strokeWidth="2">
                  ...
                  <path d="M4.5 3V1.5h3V3M2 3v7a1.5 1.5 0 001.5 1.5h5A1.5 1.5 0 0010 10V3"/>
    ...
    <>
      <div className="prf-header">
        ...
        >
          ...
          <div className="prf-img-actions" style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            ...
            {/* Eliminar banner — solo visible si hay imagen */}
            {perfil.bannerUrl && (
              <button
                className="prf-avatar-delete-btn"
                style={{ position: 'static', transform: 'none', margin: 0 }}
                title="Eliminar banner"
                onClick={() => setModal({ tipo: 'delete-banner' })}
              >
        ...
        <div className="prf-avatar-zone">
          ...
          {/* Botón editar avatar (lápiz — existente) */}
          <button
            className="prf-avatar-btn"
            title="Cambiar foto"
          ...
          {/* Botón eliminar avatar — solo si hay foto */}
          {perfil.avatarUrl && (
            <button
              className="prf-avatar-delete-btn"
        ...
        {/* ── Nombre + rol + acciones ── (sin cambios) */}
      ...
      {modal?.tipo === 'upload-banner' && (
        <UploadImageModal
          tipo="banner"
          cargando={cargando}
          onConfirm={(archivo) => handleSubir('banner', archivo)}
      ...
      {modal?.tipo === 'upload-avatar' && (
        <UploadImageModal
          tipo="avatar"
          cargando={cargando}
          onConfirm={(archivo) => handleSubir('avatar', archivo)}
          onClose={() => !cargando && setModal(null)}
        />
      )}
